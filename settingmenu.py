from PyQt5.QtWidgets import QApplication, QMainWindow, QMenu, QAction, QTreeWidgetItem, QFileDialog, QUndoCommand, QUndoStack, QInputDialog, QPushButton, QHBoxLayout, QWidget
...
class SettingMenu(QMainWindow):
    ...
    def setup(self):
        self.ui.TreeWidget.customContextMenuRequested.connect(lambda pos: menu.open_menu(self, pos))
    ...
    def add_action(self, item, type_):
        match type_:
            case "File":
                path, _ = QFileDialog.getOpenFileName(self, "Select file", desktop_path)
            case "Folder":
                path = QFileDialog.getExistingDirectory(self, "Select foler", desktop_path)
            case "Url":
                val, ok = QInputDialog.getText(self, "Url", "Entry Url", text = "https://example.com")
                if ok:
